...
def _wrap_report_page_in_scroll(page_widget, page_layout, chart_widget=None, table_widgets=()):
    ...
    for i in range(inner.count()):
        ...
        if lay is not None and lay.objectName() == "bcMainSplit":
            inner.setStretch(i, 1)
...
def _stretch_admin_bao_cao_tables(ui):
    """Tab Admin Báo cáo: bảng trong 3 panel giãn full ô (ngang + dọc)."""
    exp = QSizePolicy.Policy.Expanding
    split = getattr(ui, "bcMainSplit", None)
    if split is not None:
        for i in range(split.count()):
            split.setStretch(i, 1)
    for frame_name, lay_name, stretch_rows in (
        ("frameBcLeft", "frameBcLeftLayout", (1,)),
        ("frameBcCenter", "frameBcCenterLayout", (1, 3)),
        ("frameBcRight", "frameBcRightLayout", (1,)),
    ):
        frame = getattr(ui, frame_name, None)
        if frame is not None:
            frame.setSizePolicy(exp, exp)
            frame.setMinimumWidth(180)
        lay = getattr(ui, lay_name, None)
        if lay is not None:
            for j in stretch_rows:
                if j < lay.count():
                    lay.setStretch(j, 1)
    for tbl in (ui.tbBcTheoLop, ui.tbBcPhanBo, ui.tbBcKhoa, ui.tbBcXuat):
        ...
        if tbl.objectName() == "tbBcXuat" and ncol >= 3:
...
def _configure_admin_bao_cao_page(ui, chart_widget=None):
    ...
    _stretch_admin_bao_cao_tables(ui)
...
def bootstrap_database():
    ...
    if not schema_path.exists():
        return
    need_schema = not Path(DB_PATH).is_file() or not table_exists("TAI_KHOAN")
    if need_schema:
        conn = _db_connect()
        cur = conn.cursor()
...
class StudentApp(QWidget):
    ...
    def _update_password_strength(self, text=""):
        if getattr(self.ui, "lblPasswordStrength", None) is None or getattr(
            ...
            return
        pwd = text if isinstance(text, str) else self.ui.leNewPassword.text()
        if not pwd.strip():
...
class LecturerApp(QWidget):
    def __init__(self, username=""):
        ...
        self._stretch_lecturer_overview_tables()
    ...
    def _stretch_lecturer_overview_tables(self):
        """Tab Tổng quan: hai bảng phân công / việc cần làm chiếm full ô panel."""
        exp = QSizePolicy.Policy.Expanding
        ui = self.ui
        pq = ui.pageTongQuanLayout
        if pq.count() >= 3:
            pq.setStretch(2, 1)
        ui.overviewBottomLayout.setStretch(0, 1)
        ui.overviewBottomLayout.setStretch(1, 1)
        for frame in (ui.panel, getattr(ui, "panel1", None)):
            if frame is not None:
                frame.setSizePolicy(exp, exp)
        ui.framePhanCongLayout.setStretch(1, 1)
        ui.frameViecCanLamLayout.setStretch(1, 1)
        for tbl, col_modes in (
            (ui.tbPhanCong, (0, 1, 2, 3)),
            (ui.tbViecCanLam, (0, 1)),
        ):
            tbl.setSizePolicy(exp, exp)
            tbl.verticalHeader().setVisible(False)
    ...
    def _wire_events(self):
        ...
        self.ui.btnLuuHoSoGV.clicked.connect(self._save_lecturer_profile)
    ...
    def _load_dd_roll_table(self, selected_mon, selected_lop, hoc_ky, nam_hoc):
        ...
        tb = self.ui.tbDdNhap
        lbl = getattr(self.ui, "lblDdRoll", None)
        tb.setRowCount(0)
    ...
    def _dd_roll_apply_filter(self, _text=None):
        tb = getattr(self.ui, "tbDdNhap", None)
        if tb is None or getattr(self.ui, "txtDdTim", None) is None:
            return
    ...
    def render_overview(self):
        ...
        phan_cong_table.resizeRowsToContents()
        ...
        viec_table.resizeRowsToContents()
    ...
    def render_ho_so_tab(self):
        ...
        if table_exists("GIANG_VIEN"):
            try:
                gv = fetch_all(
                    """
                    SELECT MaGV, HoTen, Khoa, COALESCE(Email, ''), COALESCE(SoDienThoai, ''),
                           COALESCE(TrangThai, 'DANG_CONG_TAC')
                    FROM GIANG_VIEN
                    WHERE MaGV = ?
                    """,
                    (self.username,),
                ...
                gv = []
        if gv:
            ma_gv, ho_ten, khoa, email, so_dien_thoai, trang_thai = gv[0]
        else:
            ma_gv = self.username
            ho_ten = self.username
            khoa = "CNTT"
            email = f"{str(ma_gv).lower()}@abc.edu.vn"
            so_dien_thoai = ""
            trang_thai = "DANG_CONG_TAC"
        email = str(email or f"{str(ma_gv).lower()}@abc.edu.vn")
        so_dien_thoai = str(so_dien_thoai or "")
        trang_thai_label = {
            "DANG_CONG_TAC": "Đang công tác",
            "NGHI_PHEP": "Nghỉ phép",
            "NGHI_VIEC": "Nghỉ việc",
        }.get(str(trang_thai or "DANG_CONG_TAC"), str(trang_thai or ""))
        ...
        editable_rows = {1, 2, 3, 4}
        for i, (k, v) in enumerate(rows):
            tb_info.insertRow(i)
            key_item = QTableWidgetItem(k)
            key_item.setFlags(key_item.flags() & ~Qt.ItemFlag.ItemIsEditable)
            value_item = QTableWidgetItem(str(v))
            if i not in editable_rows:
                value_item.setFlags(value_item.flags() & ~Qt.ItemFlag.ItemIsEditable)
            tb_info.setItem(i, 0, key_item)
            tb_info.setItem(i, 1, value_item)
    ...
    def _save_lecturer_profile(self):
        tb = self.ui.tbThongTinGV
        if tb.rowCount() < 5:
            QMessageBox.warning(self, "Lỗi", "Không tìm thấy dữ liệu hồ sơ để lưu.")
            return
        ma_gv = self.username
        ho_ten = tb.item(1, 1).text().strip() if tb.item(1, 1) else ""
        khoa = tb.item(2, 1).text().strip() if tb.item(2, 1) else ""
        email = tb.item(3, 1).text().strip() if tb.item(3, 1) else ""
        so_dien_thoai = tb.item(4, 1).text().strip() if tb.item(4, 1) else ""
        if not ho_ten:
            QMessageBox.warning(self, "Thiếu thông tin", "Họ tên không được để trống.")
            return
        if not khoa:
            QMessageBox.warning(self, "Thiếu thông tin", "Khoa không được để trống.")
            return
        try:
            if not table_exists("GIANG_VIEN"):
                QMessageBox.warning(self, "Lỗi", "Bảng GIANG_VIEN chưa tồn tại trong cơ sở dữ liệu.")
                return
            execute_query(
            ...
            execute_query(
                "UPDATE GIANG_VIEN SET HoTen = ?, Khoa = ?, Email = ?, SoDienThoai = ? WHERE MaGV = ?",
                (ho_ten, khoa, email or None, so_dien_thoai or None, ma_gv),
            )
            self.render_ho_so_tab()
            log_event(self.username, "Giảng viên", "Quản trị", "Cập nhật hồ sơ")
            QMessageBox.information(self, "Thành công", "Đã lưu thông tin hồ sơ giảng viên.")
        except Exception as e:
            QMessageBox.warning(self, "Lỗi", f"Không lưu được thông tin hồ sơ: {e}")
...
class AdminApp(QWidget):
    ...
    def _load_lop_catalog_tab(self):
        ...
        for i, lop in enumerate(sorted(by_lop.keys())):
            ...
            btn_view.setToolTip("Xem danh sách sinh viên trong lớp")
            btn_view.clicked.connect(lambda _=False, m=str(lop): self._filter_sv_quan_ly_by_lop(m))
            btn_del = QPushButton("Xóa")
        ...
        self.ui.tbDmLop.horizontalHeader().setSectionResizeMode(2, QHeaderView.ResizeMode.Fixed)
        self.ui.tbDmLop.setColumnWidth(2, 140)
    ...
    def _on_lop_catalog_row_selected(self):
        ...
        if ma:
            self._filter_sv_quan_ly_by_lop(ma)
    ...
    def _filter_sv_quan_ly_by_lop(self, ma_lop: str):
        """Chọn lớp trên combo và nạp bảng sinh viên phía dưới."""
        ma = str(ma_lop or "").strip()
        if not ma:
            return
        cb = self.ui.cbSvQuanLyLop
        ix = cb.findData(ma)
        if ix < 0:
            cb.addItem(f"{ma}  (0 SV)", ma)
            ix = cb.findData(ma)
        if ix >= 0:
            cb.blockSignals(True)
    ...
    def _load_sv_quan_ly_theo_lop_table(self):
        tb = getattr(self.ui, "tbSvQuanLyLop", None)
        cb = self.ui.cbSvQuanLyLop
    ...
    def _load_admin_report_tab(self, *_args):
        ...
        try:
            ...
            _stretch_admin_bao_cao_tables(self.ui)
